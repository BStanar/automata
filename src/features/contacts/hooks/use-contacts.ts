...
/**
 * Hook to fetch all contacts by owner using suspense
 */
export const useSuspenseContactsByOwner = (input: { manufacturerId?: string; clientId?: string }) => {
  const trpc = useTRPC();
  return useSuspenseQuery(trpc.contacts.getByOwner.queryOptions(input));
};
...
/**
 * Hook to invalidate owners contacts
 */

export const useInvalidateOwnerContacts = (input: { manufacturerId?: string; clientId?: string }) => {
  const queryClient = useQueryClient();
  const trpc = useTRPC();

  return () => {
    queryClient.invalidateQueries({
      queryKey: trpc.contacts.getByOwner.queryKey(input),
    });
  };
};